...
import com.cornkernels.game.entities.components.plant_specific.specific_specific.PeaPodComponent;
...
public class PeaPodBehavior implements PlantAttackBehavior {
    ...
    public void execute(Entity self, Field field) {
        PeaPodComponent peaPodState = self.get(PeaPodComponent.class);

        // Self-initialize if it hasn't been added yet
        if (peaPodState == null) {
            peaPodState = new PeaPodComponent();
            self.add(peaPodState);
        }

        Vec2d origin = self.get(PositionComponent.class).position;

        for (int i = 0; i < peaPodState.headCount; i++) {

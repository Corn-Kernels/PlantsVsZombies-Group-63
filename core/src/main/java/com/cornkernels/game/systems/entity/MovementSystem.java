...
public class MovementSystem extends EntitySystem {
    ...
    public void update(float delta) {
        for (Entity e : field.getEntities()) {
            ...
            // Bypass standard movement only if the zombie is actively eating a plant.
            // ACTION states are allowed to move (e.g. charging, jumping, flying).
            if (e instanceof ZombieInstance zombie) {
                ZombieStateComponent.State state = zombie.get(ZombieStateComponent.class).state;
                if (state == ZombieStateComponent.State.EATING) {
                    continue;
                }
            }
            ...
            if (e instanceof HomingProjectile homingProj) {
                ...
                posComp.position = new Vec2d(
                    posComp.position.getX() + (velComp.velocityPerTick.getX() * 0.1f),
                    posComp.position.getY() + (velComp.velocityPerTick.getY() * 0.1f)
                );
            ...
            if (e instanceof TruePeaProjectile truePea) {
                truePea.checkTorchwood(field);
            }
        }
    }
}
